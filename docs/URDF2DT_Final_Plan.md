...
## Revised Kinematic Application Specification and Development Plan
...
**Current project scope:** Stages 0–23 cover the URDF-driven Standard-DH editor, visualization, validation, persistence, research evidence, and standalone Windows application. The project owner reduced the scope to these stages on 2026-09-20. Stage 23 is the final delivery stage in this plan; outstanding reference, usability, asset, and clean-machine acceptance checks still require evidence.
...
The DH solver must not parse or own inertia. Independent inertial extraction preserves optional source data; it does not add a dynamic model to the application. This architecture specifies planned responsibilities, not a claim that all extractors already exist.
...
inertial → InertialExtractor → InertialModel → Optional source-data export
...
The module completion gate also includes independent domain contracts and extraction, correctly attached primitive/mesh visuals with material support, distinct collision overlays and toggles, asset-warning fallback, and the modularity/transform checks in §23.6. Inertial extraction must work independently. The standalone Windows release must pass the external-asset cases in Stage 23. These kinematic application and distribution checks define the current project completion scope.

---

## 31.3 Project Definition of Done

The current project is complete when the Stage 0–23 gates are satisfied: verified kinematics, legal frame editing, independent source-data handling, visualization, validated persistence, reproducible research evidence, handoff, and a standalone Windows application verified on a clean supported machine. Reference reconciliation and technical acceptance remain explicit requirements; a candidate build alone does not establish completion.
...
24. **Independent inertial extractor** — mass, COM/inertial origin, and full symmetric inertia tensor as independently reusable source data; extraction alone is not dynamic validation.
...
The following Stage 0–23 sequence defines the current project scope. Stage 23 is the final standalone Windows delivery stage. No stage is claimed complete merely because it is listed here.
...
- Distinguish current URDF mesh/color/collision-overlay support from deferred textures, advanced CAD features, and collision engines.
...
**Done when:** on a clean supported Windows machine, a user can extract the release ZIP, double-click `URDF2DT.exe`, select a `.urdf`, and complete the full URDF-to-validated-DH-export workflow without installing development tools or dependencies. All required external-asset cases pass with source-mode-equivalent resolution and graceful missing-asset fallback. Stage 23 is the final delivery milestone for the current kinematic application scope.
...
The following ten criteria establish the kinematic contribution. Project delivery also requires the Stage 23 packaging and clean-machine acceptance checks.
...
## 35.1 Current Project Architecture

```text
URDF → Structural validation → Kinematic chain → Automatic Standard DH
     → Constrained frame editing → Local and global FK validation
     → Validated model, session and reports → Standalone Windows application
```

Independent visual, collision, material, and inertial extraction preserves source-domain boundaries. Rendering consumes geometry and FK poses without changing the accepted DH model.
...
> **URDF2DT converts a robot URDF into a validated Standard-DH model through constrained frame editing, 3-D visualization, reproducible FK checks, and session export in a standalone application.**
